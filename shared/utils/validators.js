...
function validateNote(data) {
  ...
  if (
    ...
    (typeof data.width !== 'number' || data.width < 150 || data.width > 5000)
  ) {
  ...
  if (
    ...
    (typeof data.height !== 'number' || data.height < 150 || data.height > 5000)
  ) {
  ...
  if (
    ...
    (typeof data.opacity !== 'number' || data.opacity < 50 || data.opacity > 100)
  ) {
  ...
  // Validate position values with reasonable screen bounds
  const MAX_POSITION = 32000; // Reasonable max for multi-monitor setups
  const MIN_POSITION = -32000; // Allow negative for left/top monitors

  if (data.position_x !== undefined) {
    if (typeof data.position_x !== 'number' || !Number.isFinite(data.position_x)) {
      errors.push('position_x must be a finite number');
    } else if (data.position_x < MIN_POSITION || data.position_x > MAX_POSITION) {
      errors.push(`position_x must be between ${MIN_POSITION} and ${MAX_POSITION}`);
    }
  }

  if (data.position_y !== undefined) {
    if (typeof data.position_y !== 'number' || !Number.isFinite(data.position_y)) {
      errors.push('position_y must be a finite number');
    } else if (data.position_y < MIN_POSITION || data.position_y > MAX_POSITION) {
      errors.push(`position_y must be between ${MIN_POSITION} and ${MAX_POSITION}`);
    }
  }

  // Validate z_index with reasonable bounds
  if (data.z_index !== undefined) {
    if (typeof data.z_index !== 'number' || !Number.isInteger(data.z_index)) {
      errors.push('z_index must be an integer');
    } else if (data.z_index < 0 || data.z_index > 2147483647) {
      errors.push('z_index must be between 0 and 2147483647');
    }
  }
...
function isValidNoteId(id) {
  return (
    typeof id === 'string' && id.length >= 10 && id.length <= 50 && /^[A-Za-z0-9_-]+$/.test(id)
  );
}
