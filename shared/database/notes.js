...
function updateNote(id, data) {
  ...
  const allowedFields = [
    'title', 'content', 'folder_id', 'color',
    'position_x', 'position_y', 'width', 'height',
    'is_open', 'is_pinned', 'is_locked',
    'password_hash', 'reminder_at', 'reminder_notified',
    'font_size', 'opacity', 'z_index',
  ];
  ...
  if (data.content !== undefined) {
    ...
    params.content_plain = data.content_plain !== undefined 
      ? data.content_plain 
...
function getNotesCount() {
  ...
  const archived = db.prepare('SELECT COUNT(*) as count FROM notes WHERE is_archived = 1 AND is_deleted = 0').get().count;
  const trashed = db.prepare('SELECT COUNT(*) as count FROM notes WHERE is_deleted = 1').get().count;
  const withReminder = db.prepare('SELECT COUNT(*) as count FROM notes WHERE reminder_at IS NOT NULL AND is_deleted = 0').get().count;
...
async function lockNote(id, password) {
  ...
  if (validation.strength === 'weak') {
    console.warn(`Warning: Using a weak password for note ${id}`);
  }
