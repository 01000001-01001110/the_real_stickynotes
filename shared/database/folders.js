...
function getFolderTree() {
  ...
  // Build tree with circular reference detection
  const visited = new Set();

  for (const folder of folders) {
    // Skip if already processed (prevents infinite loops from circular refs)
    if (visited.has(folder.id)) continue;
    visited.add(folder.id);

    if (folder.parent_id && map[folder.parent_id]) {
      // Circular reference check: ensure parent isn't a descendant
      let current = folder.parent_id;
      const ancestorChain = new Set();
      let isCircular = false;

      while (current && !isCircular) {
        if (ancestorChain.has(current)) {
          // Circular reference detected
          console.error(`Circular folder reference detected: ${folder.id} -> ${folder.parent_id}`);
          isCircular = true;
        } else {
      ...
      if (!isCircular) {
        ...
        // Treat circular reference folder as root
