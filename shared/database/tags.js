...
function getTagsForNotes(noteIds) {
  ...
  const validNoteIds = noteIds.filter((id) => id != null);
  ...
  // Group by note_id - initialize all requested IDs (even invalid ones) with empty arrays
  const result = {};
...
function cleanupOrphanedTags() {
  ...
  // Find tags that have no non-deleted notes
 ...
 * @returns {boolean} True if successful, false if transaction failed
 */
function setNoteTags(noteId, tagNames) {
  // Validate input
  if (!noteId) return false;
  const validTagNames = (tagNames || []).filter((name) => name && typeof name === 'string');

  try {
    withTransaction(() => {
      const db = getDatabase();

      // First collect all tag IDs we need (creates tags if they don't exist)
      const tagIds = [];
      ...
      // Only delete existing tags AFTER we've successfully created all new ones
      // This prevents data loss if tag creation fails mid-way
    ...
    console.error('Failed to set note tags:', err.message);
    return false;
  }
}
