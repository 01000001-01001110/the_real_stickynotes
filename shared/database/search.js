...
function searchByTag(tagName, options = {}) {
  ...
  // Sanitize the tag name for LIKE - escape special characters
  const sanitized = String(tagName || '')
  ...
  let sql = `
    ...
    WHERE t.name LIKE @tagName ESCAPE '\\'
  `;

  const params = { tagName: `%${sanitized}%` };
...
function getSearchSuggestions(partial, limit = 5) {
  ...
  // Get unique words from titles that match (fixed alias reference)
  const results = db
    .prepare(
      `
