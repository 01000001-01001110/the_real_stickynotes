...
function checkReminders() {
  ...
  const overdueNotes = notes.filter(note => 
    note.reminder_at && note.reminder_at <= now && !note.reminder_notified
  );
...
function showMissedRemindersSummary(notes) {
  const titles = notes.slice(0, 10).map(n => `• ${n.title || 'Untitled'}`).join('\n');
...
function getPendingReminders() {
  const now = new Date().toISOString();
  return getNotes({ withReminder: true }).filter(note => 
    note.reminder_at && note.reminder_at <= now && !note.reminder_notified
  );
}
