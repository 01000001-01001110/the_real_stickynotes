...
function setupTray(wm) {
  ...
  tray = new Tray(icon);
  tray.setToolTip('The Real StickyNotes');
  ...
  try {
    ...
    onConfigChange((changedKeys) => {
      if (changedKeys.some((k) => k.startsWith('tray.'))) {
        console.log('[Tray] Config changed, actions will update on next click');
        // Tray actions are read on each click, so no need to rebuild menu
      }
    });
  } catch (error) {
