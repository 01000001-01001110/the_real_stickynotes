...
class WindowManager {
  ...
  showPanel() {
    try {
      ...
      console.log('Creating panel window...');
      ...
      console.log('Panel HTML loaded');

      this.panelWindow.once('ready-to-show', () => {
        console.log('Panel ready to show');
        this.panelWindow.show();
      });
  ...
  calculateNotePosition(note) {
    ...
    // If note has saved position, use it
   ...
   */
  showAllNotes() {
    for (const [_, window] of this.noteWindows) {
