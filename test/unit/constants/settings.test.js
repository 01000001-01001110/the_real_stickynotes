...
describe('Settings Constants', () => {
  describe('settingsSchema', () => {
    ...
    it('should contain whisper settings', () => {
      expect(settingsSchema['whisper.enabled']).toBeDefined();
      expect(settingsSchema['whisper.modelSize']).toBeDefined();
      expect(settingsSchema['whisper.language']).toBeDefined();
      expect(settingsSchema['whisper.insertMode']).toBeDefined();
      expect(settingsSchema['whisper.defaultSource']).toBeDefined();
    });
    it('should contain VAD settings (v2 Sherpa-ONNX)', () => {
      expect(settingsSchema['whisper.vadThreshold']).toBeDefined();
      expect(settingsSchema['whisper.vadMinSilence']).toBeDefined();
      expect(settingsSchema['whisper.vadMaxSpeech']).toBeDefined();
      expect(settingsSchema['whisper.numThreads']).toBeDefined();
    });
    it('should have correct whisper defaults (v2 Sherpa-ONNX)', () => {
      expect(settingsSchema['whisper.enabled'].default).toBe(false); // Requires model installation
      expect(settingsSchema['whisper.modelSize'].default).toBe('base.en');
      expect(settingsSchema['whisper.language'].default).toBe('en');
    });
    it('should have correct VAD defaults (v2 Sherpa-ONNX)', () => {
      expect(settingsSchema['whisper.vadThreshold'].default).toBe(0.5);
      expect(settingsSchema['whisper.vadMinSilence'].default).toBe(0.25);
      expect(settingsSchema['whisper.vadMaxSpeech'].default).toBe(5.0);
      expect(settingsSchema['whisper.numThreads'].default).toBe(4);
    });
    it('should have correct VAD ranges', () => {
      expect(settingsSchema['whisper.vadThreshold'].min).toBe(0.1);
      expect(settingsSchema['whisper.vadThreshold'].max).toBe(0.9);
      expect(settingsSchema['whisper.vadMinSilence'].min).toBe(0.1);
      expect(settingsSchema['whisper.vadMinSilence'].max).toBe(2.0);
      expect(settingsSchema['whisper.vadMaxSpeech'].min).toBe(1.0);
      expect(settingsSchema['whisper.vadMaxSpeech'].max).toBe(30.0);
      expect(settingsSchema['whisper.numThreads'].min).toBe(1);
      expect(settingsSchema['whisper.numThreads'].max).toBe(8);
    });
    it('should have correct whisper insertMode default', () => {
      expect(settingsSchema['whisper.insertMode'].default).toBe('cursor');
    });
    it('should have correct whisper defaultSource default', () => {
      expect(settingsSchema['whisper.defaultSource'].default).toBe('microphone');
    });
    it('should have valid whisper modelSize options (v2 ONNX format)', () => {
      expect(settingsSchema['whisper.modelSize'].options).toContain('tiny.en');
      expect(settingsSchema['whisper.modelSize'].options).toContain('base.en');
      expect(settingsSchema['whisper.modelSize'].options).toContain('small.en');
    });
    it('should have valid whisper insertMode options', () => {
      expect(settingsSchema['whisper.insertMode'].options).toContain('cursor');
      expect(settingsSchema['whisper.insertMode'].options).toContain('append');
      expect(settingsSchema['whisper.insertMode'].options).toContain('replace');
    });
    it('should have valid whisper defaultSource options', () => {
      expect(settingsSchema['whisper.defaultSource'].options).toContain('microphone');
      expect(settingsSchema['whisper.defaultSource'].options).toContain('system');
      expect(settingsSchema['whisper.defaultSource'].options).toContain('both');
    });
