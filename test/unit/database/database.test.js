...
describe('Database Core', () => {
  ...
  beforeEach(() => {
    ...
    testDbPath = path.join(global.testDir, `test-db-${Date.now()}-${Math.random()}.db`);
  });
  ...
  describe('Schema creation', () => {
    ...
    it('should create folders table', () => {
      ...
      const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='folders'").all();
      expect(tables).toHaveLength(1);
    });
    ...
    it('should create tags table', () => {
      ...
      const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='tags'").all();
      expect(tables).toHaveLength(1);
    });
    ...
    it('should create note_tags junction table', () => {
      ...
      const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='note_tags'").all();
      expect(tables).toHaveLength(1);
    });
  ...
  describe('Basic operations', () => {
    ...
    it('should insert a record', () => {
      const stmt = db.prepare('INSERT INTO notes (id, title, content, created_at) VALUES (?, ?, ?, ?)');
      const result = stmt.run('note-1', 'Test Title', 'Test Content', new Date().toISOString());
      expect(result.changes).toBe(1);
    });

    it('should select a record', () => {
      db.prepare('INSERT INTO notes (id, title, content, created_at) VALUES (?, ?, ?, ?)')
        .run('note-1', 'Test Title', 'Test Content', new Date().toISOString());
    ...
    it('should update a record', () => {
      ...
      const result = db.prepare('UPDATE notes SET title = ? WHERE id = ?').run('Updated Title', 'note-1');
      expect(result.changes).toBe(1);
    ...
    it('should count records', () => {
      db.prepare('INSERT INTO notes (id, title, content, created_at) VALUES (?, ?, ?, ?)')
        .run('note-1', 'Title 1', 'Content 1', new Date().toISOString());
      db.prepare('INSERT INTO notes (id, title, content, created_at) VALUES (?, ?, ?, ?)')
        .run('note-2', 'Title 2', 'Content 2', new Date().toISOString());
