...
describe('Settings Options Validation', () => {
  ...
  describe('validateSettingOptions function', () => {
    ...
    it('should allow valid whisper.modelSize option (v2 ONNX format)', () => {
      const result = validateSettingOptions('whisper.modelSize', 'base.en');
      expect(result.valid).toBe(true);
    });

    it('should allow all valid whisper model size options (v2 ONNX format)', () => {
      const options = ['tiny.en', 'base.en', 'small.en'];
      options.forEach((option) => {
        const result = validateSettingOptions('whisper.modelSize', option);
        expect(result.valid).toBe(true);
      });
    });

    it('should reject invalid whisper model size option', () => {
      const result = validateSettingOptions('whisper.modelSize', 'large');
      expect(result.valid).toBe(false);
    });

    it('should reject old v1 model names', () => {
      const oldOptions = ['tiny', 'base', 'small'];
    ...
    it('should allow valid whisper.language option', () => {
      const result = validateSettingOptions('whisper.language', 'en');
      expect(result.valid).toBe(true);
    });

    it('should allow all valid whisper language options (v2 English-only)', () => {
      // v2 Sherpa-ONNX uses English-only models
      const options = ['en'];
      options.forEach((option) => {
        const result = validateSettingOptions('whisper.language', option);
        expect(result.valid).toBe(true);
      });
    });

    it('should reject invalid whisper language option', () => {
      const result = validateSettingOptions('whisper.language', 'xx');
      expect(result.valid).toBe(false);
    });

    it('should allow valid whisper.insertMode option', () => {
      const result = validateSettingOptions('whisper.insertMode', 'cursor');
      expect(result.valid).toBe(true);
    });

    it('should allow all valid whisper insert mode options', () => {
      const options = ['cursor', 'append', 'replace'];
      options.forEach((option) => {
        const result = validateSettingOptions('whisper.insertMode', option);
        expect(result.valid).toBe(true);
      });
    });

    it('should reject invalid whisper insert mode option', () => {
      const result = validateSettingOptions('whisper.insertMode', 'prepend');
      expect(result.valid).toBe(false);
    });

    it('should allow valid whisper.defaultSource option', () => {
      const result = validateSettingOptions('whisper.defaultSource', 'microphone');
      expect(result.valid).toBe(true);
    });

    it('should allow all valid whisper source options', () => {
      const options = ['microphone', 'system', 'both'];
      options.forEach((option) => {
        const result = validateSettingOptions('whisper.defaultSource', option);
        expect(result.valid).toBe(true);
      });
    });

    it('should reject invalid whisper source option', () => {
      const result = validateSettingOptions('whisper.defaultSource', 'invalid');
      expect(result.valid).toBe(false);
    });
    ...
    it('should reject whisper model size that does not exist', () => {
      const result = validateSettingOptions('whisper.modelSize', 'medium');
      expect(result.valid).toBe(false);
      expect(result.error).toContain('must be one of');
    });

    it('should reject whisper language that does not exist', () => {
      const result = validateSettingOptions('whisper.language', 'invalid');
      expect(result.valid).toBe(false);
      expect(result.error).toContain('must be one of');
    });

    it('should reject whisper insert mode that does not exist', () => {
      const result = validateSettingOptions('whisper.insertMode', 'invalid');
      expect(result.valid).toBe(false);
      expect(result.error).toContain('must be one of');
    });

    it('should reject whisper source option that does not exist', () => {
      const result = validateSettingOptions('whisper.defaultSource', 'invalid');
      expect(result.valid).toBe(false);
      expect(result.error).toContain('must be one of');
    });

    it('should reject null for whisper settings', () => {
      const result = validateSettingOptions('whisper.modelSize', null);
      expect(result.valid).toBe(false);
      expect(result.error).toContain('must be one of');
    });
  ...
  describe('setSetting with options validation', () => {
    ...
    it('should accept valid whisper option (v2 ONNX format)', () => {
      expect(() => {
        setSetting('whisper.modelSize', 'tiny.en');
      }).not.toThrow();
    });
  ...
  describe('setSettings with options validation', () => {
    ...
    it('should accept all whisper options (v2 ONNX format)', () => {
      expect(() => {
        setSettings({
          'whisper.modelSize': 'base.en',
          'whisper.language': 'en',
          'whisper.insertMode': 'append',
          'whisper.defaultSource': 'both',
        });
      }).not.toThrow();
    });

    it('should reject if whisper option is invalid', () => {
      expect(() => {
        setSettings({
          'whisper.modelSize': 'huge',
        });
      }).toThrow('must be one of');
    });
  });
  ...
  describe('Options validation coverage for all settings', () => {
    it('should validate all 9 settings with options (v2 ONNX format)', () => {
      const settingsWithOptions = [
        ...
        { key: 'whisper.modelSize', valid: 'tiny.en', invalid: 'huge' },
        { key: 'whisper.language', valid: 'en', invalid: 'badlang' },
        { key: 'whisper.insertMode', valid: 'replace', invalid: 'badmode' },
        { key: 'whisper.defaultSource', valid: 'system', invalid: 'badsource' },
      ];
